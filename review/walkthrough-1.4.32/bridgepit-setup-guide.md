...
- [Download the Mac app](https://bridgepit.com/downloads/BridgePit-mac-1.4.32.dmg). Open the DMG, drag BridgePit into Applications, then launch it.
...
The alert asked for more contracts than the strategy's Contracts setting under Strategies allows to be open. With Order fills only, TradingView sends your strategy's own order size. Set Contracts in BridgePit to the size your strategy trades, or size the strategy to match.
