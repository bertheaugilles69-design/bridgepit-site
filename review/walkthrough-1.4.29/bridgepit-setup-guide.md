...
- [Download the Mac app](https://bridgepit.com/downloads/BridgePit-mac-1.4.29.dmg). Open the DMG, drag BridgePit into Applications, then launch it.
...
1. In BridgePit, open Settings → Your public address. Choose Check again. Continue only when BridgePit confirms Tailscale connected and offers Set up public address. If it cannot finish the check, use the help above rather than starting over.
2. Choose Set up public address. If asked, choose Approve with Tailscale, complete its approval page, return to BridgePit and choose I approved — check again. Keep other Tailscale serving settings unchanged during setup.
3. Wait for Your public address is ready: BridgePit has saved the address and checked the public path. If the address already exists, choose Check and save address. An existing tunnel conflict needs review, not an automatic reset.
...
1. On the Dashboard, check Paper simulation and that Connections is still empty. If you paused entries earlier, allow them again.
...
5. In TradingView, open a chart of the instrument you chose in section 02, any timeframe. Click Pine Editor at the bottom of the chart. Replace its contents with the copied code, Save it as a private script, then Add to chart.
6. On the chart, click Alert (the clock icon). Condition: this strategy, Order fills and alert() function calls. Keep the prefilled Message. Open Notifications, switch on Webhook URL, and paste the address from BridgePit's Copy webhook URL. Check expiration and notification schedule, then create it.
7. Back in BridgePit, keep BridgePit and Tailscale running. On the next realtime market update the box under Alert setup changes to Setup confirmation received with a time. A closed market can delay this.
...
5. In TradingView, on your strategy's chart, click Alert (the clock icon). Condition: your strategy, Order fills only. Replace Message with the copied block.
...
When your strategy next fills an order in real time, TradingView sends the alert and BridgePit simulates the fill. Match that event and its Webhook status in TradingView's alert log to the FILL marked (paper) under BridgePit's Recent activity. Check the strategy, action, size and time. Backtest trades do not send these alerts.
